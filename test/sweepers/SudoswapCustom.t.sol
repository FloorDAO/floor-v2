...
contract SudoswapCustomSweeperTest is FloorTest, ERC721TokenReceiver {
    ...
    uint internal constant PROTOCOL_FEE = 0.00005 ether;
    ...
    SudoswapCustomSweeper internal sweeper;
    ...
    function setUp() public {
        ...
        sweeper = new SudoswapCustomSweeper(ASSET_RECIPIENT, PAIR_FACTORY, GDA_CURVE);
    ...
    function test_CanCreateErc721Pool() public {
        ...
        _singleCollectionExecute(address(mock721), 20 ether, 0.01 ether);
        ...
        assertEq(inputAmount, 0.01 ether + PROTOCOL_FEE, 'Invalid input amount');
        assertEq(protocolFee, PROTOCOL_FEE, 'Invalid protocol fee');
    ...
    function test_CanCreatePoolWithZeroAmount() public {
        _singleCollectionExecute(address(mock721), 0, 0.01 ether);
    }
    ...
    function test_CanFundExistingErc721Pool() public {
        ...
        _singleCollectionExecute(address(mock721), 30 ether, 0.01 ether);
    ...
    function test_CanCreateAndFundMultipleTokenPoolsInSingleTransation() public {
        ...
        uint[] memory initialPrices = new uint[](3);
        initialPrices[0] = 0.01 ether;
        initialPrices[1] = 0.1 ether;
        initialPrices[2] = 1 ether;

        // Run our multicall to fund one pool and create both a royalty-enabled
        // and non-royalty pool.
        sweeper.execute{value: 65 ether}(collections, amounts, abi.encode(initialPrices));
    }
    ...
    function test_PoolPriceUpdatesOverTime() public {
        ...
        _singleCollectionExecute(address(mock721Fees), 20 ether, 0.01 ether);
    ...
    function test_PoolSpotPriceReducesAfterEthDeposit() public {
        ...
        _singleCollectionExecute(address(mock721), 0.05 ether, 0.01 ether);
        ...
        // exploitable.
        _singleCollectionExecute(address(mock721), 100 ether, 0.01 ether);
        ...
        skip(4 weeks);
        _singleCollectionExecute(address(mock721), 10 ether, 0.01 ether);
        (,,, outputAmount,,) = pair.getSellNFTQuote(0, 1);
    ...
    function test_CanHandleDustGriefing() public {
        ...
        _singleCollectionExecute(address(mock721), 10 ether, 0.01 ether);
        _singleCollectionExecute(address(mock721Alt), 10 ether, 0.01 ether);
        ...
        _singleCollectionExecute(address(mock721Alt), 0.001 ether, 0.01 ether);
    ...
    function test_CanHandleClearingOutEntirePoolBalance() public {
        ...
        _singleCollectionExecute(address(mock721), 0.1 ether, 0.01 ether);
    ...
    function test_CanDeployWithVariedStartPrices(uint128 _startPrice) public {
        // Set the start price at a realistic value
        vm.assume(_startPrice > 0.01 ether);

        // Create a mock 721 sweep with 20 ether allocated to the pool
        _singleCollectionExecute(address(mock721), 20 ether, _startPrice);

        LSSVMPair pair = LSSVMPair(sweeper.sweeperPools(address(mock721)));
        assertEq(pair.spotPrice(), _startPrice, 'Invalid spot price');
        assertEq(pair.delta(), 324959260312412323568394994049274967, 'Invalid delta');
        assertEq(pair.fee(), 0, 'Invalid fee');
        assertEq(uint(pair.pairVariant()), 0, 'Invalid pair variant');
        assertEq(address(pair.bondingCurve()), address(sweeper.gdaCurve()), 'Invalid bonding curve');
    ...
    function _singleCollectionExecute(address _collection, uint _amount, uint128 _initialPrice) internal {
        ...
        uint128[] memory initialPrices = new uint128[](1);
        initialPrices[0] = _initialPrice;

        sweeper.execute{value: _amount}(collection, amount, abi.encode(initialPrices));
    }
