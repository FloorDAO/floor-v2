...
contract DeployStrategiesWithErc20Liquidity is DeploymentScript {
    ...
    function run() external deployer {
        ...
        IERC20(0xb81FC95DdCc666Ad8F3131aB732332424A90C3a1).approve(address(treasury), type(uint).max);
        IERC20(0xBa9c1E5bA25F48d6A1C054c3B616b614af87fc83).approve(address(treasury), type(uint).max);
        IERC20(0x785F8ddA687FdC7f24B58f6e6D8e9988f078eC79).approve(address(treasury), type(uint).max);

        // Deposit the token IDs held by the wallet
        treasury.depositERC20(0xb81FC95DdCc666Ad8F3131aB732332424A90C3a1, 8944271909999158784);   // COOL
        treasury.depositERC20(0xBa9c1E5bA25F48d6A1C054c3B616b614af87fc83, 13044262234586594242);  // DOODLE
        treasury.depositERC20(0x785F8ddA687FdC7f24B58f6e6D8e9988f078eC79, 25324049316506895273);  // PUNK

        // ~ Each one will now have 40 tokens + ETH
        ...
        approvals[0] = ITreasury.ActionApproval({
            ...
            assetContract: 0xb81FC95DdCc666Ad8F3131aB732332424A90C3a1,
            target: strategy,
            amount: 8944271909999158784
        });

        treasury.strategyDeposit(
            strategyId,
            abi.encodeWithSelector(
                NFTXLiquidityPoolStakingStrategy.depositErc20.selector,
                8944271909999158784
            ),
            approvals
        );
        ...
        approvals = new ITreasury.ActionApproval[](1);
        approvals[0] = ITreasury.ActionApproval({
            _type: TreasuryEnums.ApprovalType.ERC20,
            assetContract: 0x785F8ddA687FdC7f24B58f6e6D8e9988f078eC79,
            target: strategy,
            amount: 25324049316506895273
        });
        ...
        approvals[0] = ITreasury.ActionApproval({
            _type: TreasuryEnums.ApprovalType.ERC20,
            assetContract: 0xBa9c1E5bA25F48d6A1C054c3B616b614af87fc83,
            target: strategy,
            amount: 13044262234586594242
        });
