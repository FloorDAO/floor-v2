 ...
 */
contract SudoswapCustomSweeper is ISweeper, Ownable, ReentrancyGuard {
    ...
    {
        // Get our initial spot price from the data bytes. This will set the initial sweep price
        // when a pool is created, and when it is extended it will ensure that the price potentially
        // increase to that amount.
        uint128[] memory initialSpotPrices = abi.decode(data, (uint128[]));
        ...
        for (uint i; i < collections.length; ++i) {
            ...
            if (address(sweeperPools[collections[i]]) == address(0)) {
                ...
                sweeperPools[collections[i]] = pairFactory.createPairERC721ETH{value: amounts[i]}(
                    ...
                    0,                        // _fee
                    initialSpotPrices[i],     // _spotPrice
            ...
            // additional ETH.
                ...
                if (spotPrice > pairBalance) {
                    ...
                    if (pairBalance < initialSpotPrices[i]) {
                        pairBalance = initialSpotPrices[i];
                    }
