...
contract VeFloorStaking is IVeFloorStaking, Ownable {
    ...
    uint256 public accVeFloorPerShare;

    /// @notice Precision of `accVeFloorPerShare`
    uint256 public ACC_VEFLOOR_PER_SHARE_PRECISION;
    ...
    function setVeFloorPerSharePerSec(uint256 _veFloorPerSharePerSec) external onlyOwner {
        require(
            _veFloorPerSharePerSec <= upperLimitVeFloorPerSharePerSec,
            "VeFloorStaking: expected _veFloorPerSharePerSec to be <= 1e36"
        );
    ...
    function deposit(uint256 _amount) external {
        ...
        if (_getUserHasNonZeroBalance(_msgSender())) {
            ...
            uint256 userStakedFloor = userInfo.balance;
    ...
    function withdraw(uint256 _amount) external {
        ...
        uint256 userVeFloorBalance = veFloor.balanceOf(_msgSender());
    ...
    function getPendingVeFloor(address _user) public view returns (uint256) {
        ...
        uint256 _accVeFloorPerShare = accVeFloorPerShare;
        ...
        uint256 pendingVeFloor = pendingBaseVeFloor.add(pendingSpeedUpVeFloor);

        // Get the user's current veFLOOR balance
        uint256 userVeFloorBalance = veFloor.balanceOf(_user);

        // This is the user's max veFLOOR cap multiplied by 100
        uint256 scaledUserMaxVeFloorCap = user.balance.mul(maxCapPct);
    ...
    function _claim() private {
        uint256 veFloorToClaim = getPendingVeFloor(_msgSender());
