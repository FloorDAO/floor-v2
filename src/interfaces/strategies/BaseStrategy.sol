...
interface IBaseStrategy {
    ...
    event Withdraw(address token, uint256 amount, address caller);

    /**
     * Allows the vault to be initialised.
     */
    function initialize(uint256 _vaultId, address _vaultAddr, bytes memory initData) external;
     ...
     */
    function unmintedRewards() external returns (uint256 amount_);
     ...
     * This value is stored in terms of the `yieldToken`.
     */
    function mintedRewards() external returns (uint256 amount_);
     ...
     * This value is stored in terms of the `yieldToken`.
     */
    function rewardsAvailable() external returns (uint256 amount_);
     ...
     * This value is stored in terms of the `yieldToken`.
     */
    function totalRewardsGenerated() external returns (uint256 amount_);
     ...
     * updated accordingly.
     */
    function registerMint(uint256 amount) external;
     ...
     */
    function deposit(uint256 amount) external returns (uint256 yieldAmount_);

    /**
     * Withdraw underlying token from the strategy.
     */
    function withdraw(uint256 amount) external returns (uint256 amount_);
}
