...
interface IBaseStrategy {
     ...
     */
    function deposit(address token, uint amount) external returns (uint yieldAmount_);

    /**
     * Withdraw underlying token from the strategy.
     */
    function withdraw(address token, uint amount) external returns (uint amount_);
}
